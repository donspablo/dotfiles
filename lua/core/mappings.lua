...
local donPabloNow_options = config.options.donPabloNow
...
M.misc = function()
   ...
   local function optional_mappings()
      ...
      if not donPabloNow_options.copy_cut then
         map({ "n", "v" }, "x", '"_x')
      end

      -- don't yank text on delete ( dd )
      if not donPabloNow_options.copy_del then
         map({ "n", "v" }, "d", '"_d')
      end

      -- navigation within insert mode
      if donPabloNow_options.insert_nav then
      ...
      if donPabloNow_options.window_nav then
   ...
   local function required_mappings()
      map("n", maps.misc.cheatsheet, ":lua require('donPabloNow.cheatsheet').show() <CR>") -- show keybinds
      ...
      -- add DonPabloNowUpdate command and mapping
      cmd "silent! command! DonPabloNowUpdate lua require('donPabloNow').update_donPabloNow()"
      map("n", maps.misc.update_donPabloNow, ":DonPabloNowUpdate <CR>")
   end
